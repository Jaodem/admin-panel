...
resetForm.addEventListener('submit', async (e) => {
    ...
    if (!isPasswordValid(password)) {
        showMessage(alertContainer, 'La contraseña debe tener al menos 8 caracteres e incluir minúscula, mayúscula, número y símbolo especial', 'error')
        return;
    }
    ...
    try {
        ...
        if (!response.ok) {
            showMessage(alertContainer, data.message || 'Error al restablecer la contraseña', 'error');
            return;
        }

        showMessage(alertContainer, '¡Contraseña restablecida con éxito! Redirigiendo al login...', 'success');
        resetForm.reset();
